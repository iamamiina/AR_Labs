...
class ScanMatcher:
    ...
    def _score_alignment(self, grid: np.ndarray, scan_points: np.ndarray,
        ...
        px_new = c * px - s * py + dx
        py_new = s * px + c * py + dy
        cols = (px_new / self.local_grid_resolution).astype(int) + offset
        rows = (py_new / self.local_grid_resolution).astype(int) + offset

        # Keep only points inside grid bounds
        valid = (
